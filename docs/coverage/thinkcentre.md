...
5. Track optional-port modules, PSU/adapter choices, drive layouts, and expansion differences by chassis.

## Coverage rules

This table is an index only. Canonical YAML records under `data/thinkcentre/models/` are the source of truth. A model is not counted as researched until major specification categories and authoritative provenance are present.
